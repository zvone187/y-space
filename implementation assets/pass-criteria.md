...
Current follow-up on `y-space-browser-default-collapse` (2026-08-28): implementation and final unsigned-package verification are complete, subject only to the pre-existing Claude login and persistent Pipedream OAuth authorization gates. OpenCode and Codex now operate the visible Y Space Browser as their launch-scoped default and sole browser route, while each turn renders one closed Codex-style work disclosure with the final response left visible. The criteria below are additive; prior evidence remains retained as historical coverage.
...
- **Passed — PC-50 through PC-53 and PC-60 through PC-67:** the final package visibly ran Codex GPT-5.6 Sol and OpenCode Big Pickle against the same embedded Alpha page. Codex created a second global Browser peer, saved Alpha, reached Beta count 2, listed both tabs, closed Beta, and returned to Alpha with `Browser verified · 11 actions`; OpenCode then found the surviving tab, saved `QA-FINAL-OPENCODE-LATEST`, and returned `Browser verified · 6 actions`. The Codex rollout contains 15 nested `mcp__browser__*` calls through the isolated Code Mode host and no browser work through shell or an external process. Launch-profile, filter-proxy, indirection-gate, WSL, reload, ownership, and process-group regressions all pass.
- **Passed — PC-55 through PC-59:** both final live runs kept intermediate work behind one closed **Working** row. They settled to closed **Worked · 38 steps** and **Worked · 24 steps** rows with only the final answers expanded. Computer Use expanded and collapsed both histories successfully, and the complete Node 24 suite passed 912 files/10,343 tests with 5 files/85 tests skipped.
...
- **Verified with disclosed viewport caveat — PC-49:** the final source state passed 912 files and 10,343 tests with 5 files/85 tests skipped in 126.46 seconds under Node 24.13.0. Typecheck, lint, type-aware lint, format (3,031 files), desktop production build, thin-arm64 packaging, and deep/strict ad-hoc codesign passed. The final screenshots capture a 1,219×768 content frame; narrower behavior is covered by responsive regressions, not presented as a separate final 1,024×768 manual capture.
...
Final package evidence: `release/Y-Space-1.6.6-arm64.dmg`, 156,279,617 bytes, SHA-256 `306a7143eb59a682aa8532b8835e7e83f239b7a4b73243b61136f2e188a32ffa`, thin arm64, valid ad-hoc/hardened-runtime signature, no Team ID. Packaged `app.asar` SHA-256 is `86537dbeb79f1df3cd1124f0729a8d0bd0ec8207c5005b1762b2a175ffd1cb16`; the executable SHA-256 is `02a4c01b2c7adbdb277b7f8fe8bcc7c3fe67388a30d69a23c374c8f834508b8a`.
