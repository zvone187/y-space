...
- When the managed Y Space Browser is connected, make it the provider's only web route at launch: suppress provider-native web/search, competing browser MCPs/skills, and common browser-driver commands without mutating the user's saved provider profiles or unrelated tools.
