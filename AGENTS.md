...
- When the managed Y Space Browser is connected, make it the provider's only web route at launch: suppress provider-native web/search, competing browser MCPs/skills, and common browser-driver commands without mutating the user's saved provider profiles. Preserve app-managed unrelated MCPs and Pipedream relays; isolate unmanaged provider-profile MCPs launch-locally when their advertised tools cannot be safely filtered before launch.
...
- File-based Pipedream setup is a user-confirmed one-time secure move: require a signed packaged macOS identity with the real app-isolated Keychain (never mock keychain), durably seal a pending record before removing the dedicated plaintext file, sync its directory, then durably activate the sealed record. Refuse destructive import on Windows, Linux, development/unsigned builds, or session-only storage; never bookmark or auto-read workspace credential files.
- Never accept Pipedream credentials or setup paths through the desktop/headless exec environment. A tainted desktop process must scrub and terminate immediately before showing any notice from a clean relaunch; no supervisor, provider, or window may start beside those immutable exec-time values.
- Personal Pipedream OAuth may persist only behind the same verified Developer ID/macOS Keychain boundary. On all weaker builds it is session-only, and legacy Personal tokens must be durably removed before any agent starts.
- Keep trusted JavaScript and MJS inside the integrity-checked ASAR. `asarUnpack` is for required native binaries only; release fuses must reject Node options and inspector CLI arguments, and macOS entitlements must not allow DYLD injection or disable library validation.
- Every Y Space desktop test launch—development or packaged—must use an isolated `PORACODE_BASE_DIR` and the explicit `--use-mock-keychain` CLI flag, then keep that same process open between test cases. For packaged QA, launch the exact test bundle. Never enable the mock keychain in ordinary release configuration or launches.
...
- Preserve the default Y Space visual language: native system sans for product chrome, near-white structural surfaces, sparse bright-orange fills with separate accessible text/foreground tokens, and translucency only on stable chrome behind a contrast-safe scrim. Monospace is reserved for code, commands, paths, IDs, diffs, and terminals. Keep `prefers-reduced-transparency` fallbacks; never add backdrop blur per row, per tab, or inside embedded browser/document content.
- Keep Tailwind's Vite optimizer disabled and let Vite/esbuild minify CSS until a packaged Electron check proves another pipeline preserves the standard `backdrop-filter` declaration. Tailwind 4.3.2's Lightning CSS pass retains only `-webkit-backdrop-filter`, which Electron 43 does not expose and silently degrades every glass surface to transparency.
